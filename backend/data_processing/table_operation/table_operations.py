...
def join_dataframes(
    ...
    """
    try:
        # Normalize join keys to lists for consistent processing
        left_on = [left_on] if isinstance(left_on, str) else left_on
        right_on = [right_on] if isinstance(right_on, str) else right_on

        # Ensure join columns are included in column selections
        if left_columns is not None:
            left_columns = list(set(left_columns + left_on))
        if right_columns is not None:
            right_columns = list(set(right_columns + right_on))

        # Filter DataFrames to include only specified columns
        if left_columns is not None:
...
def stack_dataframes(
    ...
    """
    if len(dataframes) < 2:
        raise ValueError("At least two DataFrames are required to perform stacking operation")
